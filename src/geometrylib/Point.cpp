...
namespace geo
{
	...
	int Point::Quadrant(Pointer<Point> p) const
	{
		if (p->x < this->x)
		{
			if (p->y < this->y)
	...
	void Point::Rotate(Pointer<Point> p, double angle)
	{
	...
	void Point::Rotate(const geo::Point &p, double angle)
	{
		double currentAngle = Calc::GetAngle(p, *this);
		angle += currentAngle;
		if (angle < 0) {angle += (M_PI * 2);}
